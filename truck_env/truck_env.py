...
class HierarchicalTruckRoutingEnv(MultiAgentEnv):
    ...
    def __init__(self, config=None):
        ...
        print("I'm Here")
        self._raw_obs_space = get_observation_space(
            self.graph)  # , self.num_trucks)
    ...
    def step(self, action_dict):
        ...
        for i, truck in enumerate(self.trucks):
            ...
            if high_agent in action_dict:
                print(
                    f"🔁 High Agent {high_agent} executing action {action_dict[high_agent]}")
                target_node = action_dict[high_agent]
                self._execute_route_action(
                    truck, target_node, rewards, terminateds)
        ...
        for i, truck in enumerate(self.trucks):
            ...
            if low_agent in action_dict:
                print(
                    f"🔁 Low Agent {low_agent} executing action {action_dict[low_agent]}")
                charge_action = action_dict[low_agent]
                self._execute_charge_action(truck, charge_action, rewards)
            print(
                f"🔋 Truck {truck['id']} battery: {truck['current_battery']:.2f}")
        # Update global time
    ...
    def _execute_route_action(self, truck, target_node, rewards, terminateds):
        ...
        travel_time = edge_data["distance"] / \
            self.truck_types[truck["truck_type"]]["base_speed"]

        # Calculate discharge
        discharge = discharge_function(
            truck, edge_data, travel_time, self.global_time)
        '''
    ...
    def _can_reach_destination(self, truck):
        ...
        try:
            ...
            estimated_discharge = self.truck_types[truck["truck_type"]]["base_discharge_function"](
